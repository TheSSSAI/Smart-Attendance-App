...
/**
 * Zod schema for validating the input data for approving an attendance record.
 */
const ApproveAttendanceDataSchema = z.object({
  recordId: z.string().min(1, "recordId is required."),
});

/**
 * Data Transfer Object (DTO) for approving a single attendance record.
 * This class encapsulates the data required for the ApproveAttendanceUseCase.
 * It includes validation logic to ensure data integrity at the application boundary.
 */
export class ApproveAttendanceDTO {
  /**
   * The unique identifier of the attendance record to be approved.
   */
  ...
  /**
   * Creates and validates an ApproveAttendanceDTO from the raw input of a callable function.
   ...
   * @returns A validated instance of ApproveAttendanceDTO.
   */
  public static create(data: unknown, context: CallableContext): ApproveAttendanceDTO {
    if (!context.auth) {
    ...
    const validationResult = ApproveAttendanceDataSchema.safeParse(data);
    if (!validationResult.success) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "Invalid data provided for attendance approval.",
    ...
    return new ApproveAttendanceDTO(validatedData.recordId, actorValidation.data);
  }
}