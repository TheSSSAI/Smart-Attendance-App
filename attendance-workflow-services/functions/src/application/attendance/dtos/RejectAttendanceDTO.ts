...
/**
 * Zod schema for validating the input data for rejecting an attendance record.
 * A reason is mandatory and must meet a minimum length requirement.
 */
const RejectAttendanceDataSchema = z.object({
...
/**
 * Data Transfer Object (DTO) for rejecting a single attendance record.
 * This class encapsulates the data required for the RejectAttendanceUseCase.
 * It includes validation logic to ensure data integrity at the application boundary.
 */
export class RejectAttendanceDTO {
  /**
   * The unique identifier of the attendance record to be rejected.
   */
  public readonly recordId: string;

  /**
   * The justification provided by the supervisor for rejecting the record.
   */
  ...
  /**
   * Creates and validates a RejectAttendanceDTO from the raw input of a callable function.
   ...
   * @returns A validated instance of RejectAttendanceDTO.
   */
  public static create(data: unknown, context: CallableContext): RejectAttendanceDTO {
    if (!context.auth) {
    ...
    const validationResult = RejectAttendanceDataSchema.safeParse(data);
    if (!validationResult.success) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "Invalid data provided for attendance rejection.",
    ...
    return new RejectAttendanceDTO(
